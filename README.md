# glue
glue is a Python library with the most frequent command-line Python application tools.
It is compatible with both Python 2.7 and 3.5+.

## Command line parser
`glue` contains command line parser with autocompletion,
which simplifies and speeds up developing simple command-line applications.

You only bind a keyword to the function and `glue` is doing all the rest for you:
...
* It adds bash autocompletion
(by pressing `Tab` you get the most relevant CLI arguments hints and autocompletion)
* You can add custom auto-completers for your commands
* It supports multilevel subcommands ('git style alike' syntax, i.e. `git push origin master`)
* It supports both parameters format: `--param value` or `--param=value`
...
Run in terminal any of the commands:

`./sample_app.py`, `./sample_app.py -h`, `python sample_app.py --help` or `./sample_app.py --help`.
...
In order to autocompletion takes effect, there must be a specific script in `/etc/bash_completion.d/`.
With glue you just need to run:
```shell
# sudo ./sample_app.py --bash-install sample_app
```
It will install autocompletion script and adds a symbolic link in `/usr/bin/`,
...
If you type `sample_app hel` it will automatically fill the only possible option (`hello`).
